...
void Protocol::requeue_up_to(seq_num_t seq)
{
  ...
  if (stopped) {
    ...
    logger().debug("{} nothing to requeue according to seq {}, out_seq {} unchanged",
                   conn, seq, conn.out_seq);
  }
}
...
seastar::future<stop_t> Protocol::do_write_dispatch_sweep()
{
  switch (write_state) {
   case write_state_t::open: {
    ...
    auto acked = ack_left;
    assert(!acked || (acked && conn.in_seq));
    // sweep all pending writes with the concrete Protocol
    return socket->write(do_sweep_messages(
        conn.pending_q, num_msgs, need_keepalive, keepalive_ack, bool(acked))

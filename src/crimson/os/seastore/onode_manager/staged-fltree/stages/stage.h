...
struct staged {
  ...
  class _iterator_t<CTYPE, std::enable_if_t<CTYPE == ContainerType::INDEXABLE>> {
   ...
   public:
    ...
    std::enable_if_t<IS_BOTTOM, const T*> insert(
        ...
        const value_input_t& value, node_offset_t insert_size, const char* p_left_bound) {
  ...
  class _iterator_t<CTYPE, std::enable_if_t<CTYPE == ContainerType::ITERATIVE>> {
   ...
   public:
    ...
    template <KeyT KT>
    static node_offset_t estimate_insert(const full_key_t<KT>& key, const value_input_t& value) {
      return container_t::template estimate_insert<KT>(key, value);
    }
  ...
  template <KeyT KT>
  static node_offset_t insert_size(const full_key_t<KT>& key, const value_input_t& value) {
    if constexpr (IS_BOTTOM) {
  ...
  static node_offset_t insert_size_at(
      match_stage_t stage, const full_key_t<KeyT::HOBJ>& key, const value_input_t& value) {
    if (stage == STAGE) {

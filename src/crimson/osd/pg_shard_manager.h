...
public:
  ...
  auto process_ordered_op_remotely(
      ...
      typename T::IRef &&op,
      F &&f) {
    auto &crosscore = get_osd_priv(&op->get_connection()).crosscore;
    if (crosscore.proceed_or_wait(cc_seq)) {
      ...
      logger.debug("{} got {} at the remote pg, wait at {}",
                   *op, cc_seq, crosscore.get_in_seq());
  ...
  auto with_remote_shard_state_and_op(
    ...
    auto &opref = *op;
    auto &crosscore = get_osd_priv(&opref.get_connection()).crosscore;
    auto cc_seq = crosscore.prepare_submit(core);
    auto &logger = crimson::get_logger(ceph_subsys_osd);
